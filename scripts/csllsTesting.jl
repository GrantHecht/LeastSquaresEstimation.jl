...
function main()
    ...
    xhat0 = zeros(n)
    P0    = zeros(n, n)
    map(i -> P0[i,i] = 10.0, 1:n)
    cslse = SequentialLinearLeastSquares(h, xhat0, P0, CovarianceForm())
    ...
    for step in 1:length(batchSteps)
        ...
        FeedMeasurementBatch!(cslse, ys[idxs], W, ts[idxs])

        # Compute estimate 
        ComputeEstimate!(cslse)

        # Compute estimated measurements and plot 
        yhat = zeros(m)
        @inbounds for i in 1:m
            yhat[i] = GetMeasurementEstimate(cslse, ts[i])
        end

        # Display 
        println("Estimated parameters for update $step:")
        display(GetEstimate(cslse))
